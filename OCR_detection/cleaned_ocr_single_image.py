...
# List of logos
logos = ['betway', 'berlin', 'aok', 'hyundai']  # Ensure all logos are in lowercase
...
for line in result[0]:
    ...
    if processed_text in logos:
        ...
        print(f"exact match Detected Text: {text}, Confidence: {confidence:.2f}, Bounding Box: {bbox}")
    else:
        # Find the closest match with a similarity threshold of 0.6
        best_match = get_close_matches(processed_text, logos, n=1, cutoff=0.6)
        if best_match:
            # Append the bbox, text, and confidence in the required format
            original_results[0].append((bbox, (best_match[0], confidence)))
            print(f"best match Detected Text: {best_match[0]}, Confidence: {confidence:.2f}, Bounding Box: {bbox}")
