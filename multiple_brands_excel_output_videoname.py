...
class DownloadManager:
    ...
    Generates an Excel file listing URLs with priority, associated logos, and video name.
    """
    ...
    def process_json_file(self, json_file):
        ...
        for idx, item in enumerate(data):
            ...
            medium = item.get("medium")  # Extract the medium (video name)

            if download_url and brand_name and logo_id and medium:
                # Create a combined key using brandName_logoId
                brand_logo_key = f"{brand_name}_{logo_id}"

                # Map the download URL to the brandName_logoId and medium (video name)
                if download_url not in self.url_to_brands:
                    self.url_to_brands[download_url] = {
                        "brand_logos": [],
                        "video_name": medium
                    }
                self.url_to_brands[download_url]["brand_logos"].append(brand_logo_key)
    ...
    def prioritize_downloads(self, target_brand_logo):
        ...
        """
        for url, data in self.url_to_brands.items():
    ...
    def export_to_excel(self, save_path):
        """
        Exports the priority lists to an Excel file with an additional 'Logos' and 'Video Name' columns.
        ...
        for url in self.high_priority:
            logos = ', '.join(self.url_to_brands[url]["brand_logos"])  # Join the list of logos as a string
            video_name = self.url_to_brands[url]["video_name"]
            all_data.append({'Priority': 'High', 'download_url': url, 'Video Name': video_name, 'Logos': logos})

        for url in self.medium_priority:
            logos = ', '.join(self.url_to_brands[url]["brand_logos"])
            video_name = self.url_to_brands[url]["video_name"]
            all_data.append({'Priority': 'Medium', 'download_url': url,'Video Name': video_name, 'Logos': logos})

        for url in self.low_priority:
            logos = ', '.join(self.url_to_brands[url]["brand_logos"])
            video_name = self.url_to_brands[url]["video_name"]
            all_data.append({'Priority': 'Low', 'download_url': url,'Video Name': video_name ,'Logos': logos})
